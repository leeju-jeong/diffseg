...
class TextGuidedKDLoss(nn.Module):
    """
    Text-Guided Knowledge Distillation Loss
    
    Teacher와 Student의 text similarity map을 MSE로 비교
    """
    ...
    def __init__(self, lamb: float = 0.1, max_value: float = 10.0):
        ...
        """
        super(TextGuidedKDLoss, self).__init__()
        self.lamb = lamb
    ...
    def forward(self, 
        ...
        """
        Args:
            student_similarity: (B, C, H, W) Student similarity map
            teacher_similarity: (B, C, H, W) Teacher similarity map
            mask: (B, 1, H, W) or (B, H, W) Valid pixel mask
        
        Returns:
            loss: Scalar loss value
        """
        assert student_similarity.shape == teacher_similarity.shape, \
            f"Shape mismatch: {student_similarity.shape} vs {teacher_similarity.shape}"
        ...
        if mask is not None:
            ...
            # Expand to match similarity shape
